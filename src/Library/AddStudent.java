...
public class AddStudent extends JDialog implements ActionListener{
	...
	public AddStudent() {
		...
		lblStudentId.setBounds(23, 275, 92, 25);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(ob == submit){
			if((tname.getText().length())!=0 && (tage.getText().length())!=0 && (m.isSelected() || f.isSelected()) && (tmob.getText().length())!=0 && (tadd.getText().length())!=0 && (tuname.getText().length())!=0 && (tpass.getText().length())!=0){
				...
				String s = "insert into studentdetails(id,name,uname,pass,age,gender,mobno,address) values(?,?,?,?,?,?,?,?)";
	...
	void generateId(){
		String s = "select max(id) from studentdetails";
		...
		tid.setText(String.valueOf(sid));
	}
}
