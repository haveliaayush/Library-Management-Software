...
public class AddTeacher extends JDialog implements ActionListener{
	...
	public AddTeacher() {
		...
		tid1.setBackground(Color.WHITE);
		tid1.setColumns(10);
		tid1.setBounds(145, 277, 161, 25);
		getContentPane().add(tid1);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(ob == submit){
			if((tname.getText().length())!=0 && (tage.getText().length())!=0 && (m.isSelected() || f.isSelected()) && (tmob.getText().length())!=0 && (tadd.getText().length())!=0 && (tuname.getText().length())!=0 && (tpass.getText().length())!=0){
				...
				String s = "insert into teacherdetails(id,name,uname,pass,age,gender,mobno,address) values(?,?,?,?,?,?,?,?)";
	...
	void generateId(){
		String s = "select max(id) from teacherdetails";
		...
		tid1.setText(String.valueOf(tid));
	}
}
